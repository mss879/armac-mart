 ...
 * The banner is a promo strip stacked above one real nav. It retracts on
 * scroll-down, returns on scroll-up, and dismisses via ×. Banner and bar stay
 * visually distinct through fill contrast, so the page never reads as two navs.
 */
...
export function SiteHeader() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // Only retract once the banner has genuinely scrolled past.
      if (y > 96 && y > lastY.current) setRetracted(true);
      else if (y < lastY.current) setRetracted(false);
      lastY.current = y;
    };
  ...
  return (
    <header className="sticky top-0 z-30">
      {bannerOpen && (
        <div
          className={`overflow-hidden bg-walnut text-accent-ink transition-[grid-template-rows] duration-[240ms] ease-out grid ${
            retracted ? "grid-rows-[0fr]" : "grid-rows-[1fr]"
          }`}
        >
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-1.5 lg:ml-6">
            ...
            >
              ...
              {/* Label sheds below sm so the bar fits a 320px viewport */}
